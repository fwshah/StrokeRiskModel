...
- No model load at import time. Uses lazy cache (st.cache_resource if available).
- Clear diagnostics printed to Streamlit Cloud logs on failure (sklearn + Python versions).
- Safe feature handling: aligns DataFrame to model.feature_names_in_ when available.
- Imports heavy libs (e.g., seaborn, matplotlib) only when needed to avoid import-time crashes.
- Paths resolved relative to this file; supports env override for MODEL_FILE.
"""
...
# --- Optional Streamlit-aware caching (falls back if not running under Streamlit) ---
try:
    import streamlit as st
    _cache_resource = st.cache_resource
except Exception:
    st = None  # not running in Streamlit context
    # simple cache decorator fallback
...
# ------------------------------
# Defaults for preprocessing (keep your original stats)
# ------------------------------
age_mean, age_std = 43.23, 22.61
glucose_mean, glucose_std = 106.15, 45.28
bmi_mean, bmi_std = 28.89, 7.85
...
# This is the baseline feature order used if the model doesn't expose names.
...
def get_model():
    """
    Lazy-load and cache the model. Prints diagnostics to stdout (Cloud logs).
    Returns:
        (model, feature_names_list_or_None)
    Raises:
    ...
    try:
        ...
        print("[ERROR] joblib.load failed")
        traceback.print_exc()
        raise
    ...
    # Multiple possible save formats
    if isinstance(obj, dict):
        model = obj.get("model", None)
        feature_columns = obj.get("feature_columns", None)
        if model is None:
            # fall back: first estimator-like value
            for v in obj.values():
    ...
    # If the estimator provides feature names, prefer them
    try:
    ...
    if feature_columns:
        print("Feature columns from model:", len(feature_columns))
    else:
        print("No feature columns found on model; using DEFAULT_FEATURE_ORDER")
